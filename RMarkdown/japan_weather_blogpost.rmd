...
Before we get into anything else I will download Japan polygons using the `jpndistrict` package:
...
First using the `jpn_pref()` function from the `jpndistrict` package we specify the spatial polygons data we want to grab. In the `pref_code` argument we pass **13** as that's the code for the Tokyo Prefecture. 

```{r tokyo_prefecture_polygons}
...
From here we can use the weather stations metadata from the `jmastats` package and filter for stations located in Tokyo. We'll select only those that are of type "四" or the AMeDAS stations that record temperature, precipitation, wind, and sunshine duration because this is the type of station I'll be using in the other visualizations. Just a little note on what AMeDAS is:
...
Now that we got a little bit of context we can move on to the viz stuff!
...
The ggplot2 code I used was fairly straightforward, a nifty new package I started using was the `glue` package by Jim Hester, basically to get the text to set nicely in the plot. We'll see more uses of this in the other visualizations as well!
...
## Grabbing Japanese Weather Data ft. RIEM & JMAStats
...
ggplot(j_temp_map_stations_df) +
  geom_line(aes(x = date, y = temperature_average, color = prefecture)) +
  gghighlight(max(temperature_average) > 25)
...
- use `fill()` and direction = "up" for NA/missing to take the previous day's temperature
- create english names for `jpndistrict::jpnpref` data set
...
j_temp_map_stations_df %>% 
  ...
  fill(temperature_average, .direction = "up") %>% 
  group_by(region) %>% 
  mutate(region_avg_temp = mean(temperature_average)) %>% 
  ungroup() %>% 
  filter(region == "関東") %>% 
  ggplot(aes(x = temperature_average, y = prefecture)) +
...
j_temp_map_stations_df %>% 
  rename(prefectures = prefecture) %>% 
  ggplot(aes(date, temperature_average)) +
  geom_line() +
  facet_geo(~ prefectures, 
            grid = "jp_prefs_grid1")
```
