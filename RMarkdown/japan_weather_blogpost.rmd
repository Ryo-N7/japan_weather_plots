...
The two places I know where to get Japan weather data using R is via the `RIEM` and `JMAStats` packages. In this section I'll go through what you need to do to get the data.
...
If we use `geom_line()` to check out how each of the prefectures stack up, it's really hard to interpret as there are so many lines! We could just filter for the top 5 but then we lose context as we can't see those in relation to the other prefectures. So, the solution here is the `gghighlight` package.

We can use the `gghighlight()` function and we specify the filtering variable as the average of the average temperatures and with the `max_highlight` argument we can define the exact number of prefectures we want.
...
j_temp_colors <- j_temp_stations_df %>% 
  # remove the prefecture suffixes
...
Then set up the colors for each region and plot:
